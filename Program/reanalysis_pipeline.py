...
def problem_8(galaxy_data: np.ndarray):
    ...
    print(f"Galaxy Region Lower/Upper Flux Limits: {lower_flux_limit}, {upper_flux_limit}")
...
def problem_9(galaxy_data: np.ndarray, zero_velocity_row: int):
    ...
    n_above = (zero_velocity_row - half_stripe) // stripe_size
    n_below = (y_size - 1 - (zero_velocity_row + half_stripe)) // stripe_size
